...
$env:HNODECB_STAGE2_STEP_MAX_LOSS_INCREASE_FRAC = "0.05"
...
$env:HNODECB_STAGE2_LOG_SUBDIR = "stage2_step2a/local/Full_horizon"
$env:HNODECB_STAGE2_LOG_PREFIX = "log2_03_step2a_stage2light_full_horizon_local"

Remove-Item Env:HNODECB_STAGE2_SHARD_INDEX -ErrorAction SilentlyContinue
Remove-Item Env:HNODECB_STAGE2_SHARD_LOG_SUBDIR -ErrorAction SilentlyContinue

if (-not (Test-Path -Path (Join-Path $repoRoot "logs"))) {
  New-Item -ItemType Directory -Path (Join-Path $repoRoot "logs") | Out-Null
}
if (-not (Test-Path -Path (Join-Path $repoRoot "logs\\stage2_step2a\\local\\Full_horizon"))) {
  New-Item -ItemType Directory -Path (Join-Path $repoRoot "logs\\stage2_step2a\\local\\Full_horizon") -Force | Out-Null
}

$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$stage2Log = Join-Path $repoRoot ("logs\\stage2_step2a\\local\\Full_horizon\\log2_03_step2a_stage2light_full_horizon_local_driver_" + $timestamp + ".txt")
$stage2Runner = Join-Path $repoRoot "runner\\stagewise\\afm_param_search_stage2light_03.jl"

if ($Shards -ne 1) {
  Write-Host "Note: full-horizon Stage2light forces SHARDS=1; requested Shards=$Shards is ignored."
}
Write-Host "Running Stage2light FULL-HORIZON (03) with JULIA_NUM_THREADS=$env:JULIA_NUM_THREADS, BLAS_THREADS=$env:OPENBLAS_NUM_THREADS, SELFTEST=$env:HNODECB_SELFTEST, SHARDS=$env:HNODECB_STAGE2_SHARD_COUNT, CANDIDATE_RANK=$env:HNODECB_STAGE2LIGHT_CANDIDATE, INIT_RETRIES=$env:HNODECB_STAGE2_INIT_RETRIES, MAXITERS=$env:HNODECB_STAGE2_MAXITERS, OPT=$env:HNODECB_STAGE2_OPTIMIZER, G_SCALE=$env:HNODECB_STAGE2_GRAD_SCALE_P_NET/$env:HNODECB_STAGE2_GRAD_SCALE_MECH, GROUP_ADAPT=$env:HNODECB_STAGE2_GROUP_ADAPT, GROUP_ETA=$env:HNODECB_STAGE2_GROUP_ETA, LR_AGGR_KEEP=$env:HNODECB_LR_AGGRESSIVE_KEEP, LR_AGGR_UP=$env:HNODECB_LR_AGGRESSIVE_UP"
Write-Host "Repo root  -> $repoRoot"
Write-Host "Project    -> $projectPath"
Write-Host "Driver log -> $stage2Log"
Write-Host "Print dir  -> logs\\stage2_step2a\\local\\Full_horizon"
