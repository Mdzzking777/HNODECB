...
from AFM05.stage1pluslight.data import initial_condition_aligned_dataset, load_dataset
...
from AFM05.stage2light.rollout import LearnableMechModule, rollout_single_shooting_torch
...
def _build_model_and_mech(payload: dict) -> tuple[KANForceModule, LearnableMechModule, tuple[float, ...], torch.dtype]:
    ...
    ks0 = float(np.sqrt(max(float(cfg["ks_lo"]), 1.0e-30) * max(float(cfg["ks_hi"]), 1.0e-30)))
    cs0 = float(np.sqrt(max(float(cfg["cs_lo"]), 1.0e-30) * max(float(cfg["cs_hi"]), 1.0e-30)))
...
def _relative_rmse_pct_np(pred: np.ndarray, reference: np.ndarray) -> float:
    pred_arr = np.asarray(pred, dtype=float)
    reference_arr = np.asarray(reference, dtype=float)
    count = max(int(reference_arr.size), 1)
    err = float(np.sum(np.square(pred_arr - reference_arr)))
    den = float(np.sum(np.square(reference_arr)))
    reference_rms = np.sqrt(den / float(count))
    if reference_rms <= 0.0:
        return 0.0
    return 100.0 * np.sqrt(err / float(count)) / reference_rms
...
def main() -> None:
    ...
    for ax, (name, start, stop, indices) in zip(flat_axes, panels, strict=False):
        ...
        ax.plot(times_eval_us[pos], ode_rollout_true[0, indices], color="black", linewidth=2.0, label="x1 observed", zorder=1)
        ax.plot(
            times_eval_us[pos],
    ...
    fig.suptitle(
        "AFM05 stage2light postprocess rollout from W0 known IC\n"
    ...
    finalize_and_save(fig, OUT_DIR / "afm_param_stage2light_05_x1_full_rollout_grid.png")
