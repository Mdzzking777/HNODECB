"""CLI entrypoint for the standalone AFM05 Python stage2light library."""
...
from AFM05.stage2light.archive import archive_completed_stage2light_run
from AFM05.stage2light.config import default_config
from AFM05.stage2light.train import merge_stage2light_results, run_stage2light_shard
...
def _driver_log_path(cfg) -> Path:
    ...
    return cfg.log_dir / f"log2_05_step2a_stage2light_local_driver_{stamp}.txt"


def _shard_log_path(cfg, shard_index: int) -> Path:
    return cfg.shard_log_dir / f"log2_05_step2a_stage2light_local_p{shard_index}.txt"
...
def run_driver(cfg, *, auto_generate_dataset: bool) -> dict[str, object]:
    ...
    base_env["HNODECB_AFM05_STAGE2LIGHT_SHARD_COUNT"] = str(cfg.shard_count)
    if auto_generate_dataset:
        base_env["HNODECB_AFM05_STAGE2LIGHT_AUTOGEN_DATASET"] = "1"

    with driver_log.open("w", encoding="utf-8") as log:
        _log_line(log, f"AFM05 stage2light driver start | shard_count={cfg.shard_count}")
        ...
        for shard_index in range(1, cfg.shard_count + 1):
            ...
            env = dict(base_env)
            env["HNODECB_AFM05_STAGE2LIGHT_SHARD_INDEX"] = str(shard_index)
            proc = subprocess.Popen(
                [sys.executable, "-m", "AFM05.stage2light.main", "--shard-index", str(shard_index)],
        ...
        if failed:
            ...
            raise RuntimeError(f"AFM05 stage2light driver failed: {failed}")
