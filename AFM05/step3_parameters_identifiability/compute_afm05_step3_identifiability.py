"""AFM05 step3 identifiability computation from a completed stage2light result.

This script implements the AFM05 counterpart of the HNODECB step3 idea:
...
The formal AFM05 default parameter set is "trained": active trainable KAN
...
from AFM05.stage2light.kan_backend import KANForceModule  # noqa: E402
from AFM05.stage2light.rollout import (  # noqa: E402
    LearnableMechModule,
...
CURRENT_ENTRY_SCHEMA = "afm05_step3_current_entry_v1"
RESULT_SCHEMA = "afm05_step3_identifiability_v2"
SHARD_SCHEMA = "afm05_step3_identifiability_shard_v1"
...
def raw_rms(values: np.ndarray) -> float:
    arr = np.asarray(values, dtype=float).reshape(-1)
    if arr.size == 0:
        return 0.0
...
def build_model(payload: dict[str, Any]) -> tuple[KANForceModule, dict[str, Any], torch.dtype]:
    cfg = payload["config"]
    dtype = torch_dtype(cfg["dtype"])
    device = "cpu"
    known_pars = payload["known_pars"]
    if not isinstance(known_pars, dict):
        raise TypeError("AFM05 stage2light result requires dict-valued known_pars")
    model = KANForceModule(
        ...
        dist=float(known_pars["Z"]),
        a0=float(known_pars["a0"]),
...
def build_mech_from_state(payload: dict[str, Any], dtype: torch.dtype) -> LearnableMechModule:
    ...
    mech = LearnableMechModule(
        ks_init=math.sqrt(ks_lo * ks_hi),
        cs_init=math.sqrt(cs_lo * cs_hi),
        ks_bounds=(ks_lo, ks_hi),
        cs_bounds=(cs_lo, cs_hi),
        dtype=dtype,
        device="cpu",
...
def trained_parameter_refs(
    ...
    """Return AFM05 formal step3 trainable parameters, excluding u0.
    ...
    if not refs:
        raise RuntimeError("No trainable parameters found for AFM05 step3")
    return refs
...
def final_snapshot_arrays(payload: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    """Return the exact sampled time grid analyzed at the final st2l epoch.

    AFM05 window_meta stores the raw index span, while final_snapshot stores the
    stride-sampled 393-point rollout used by stage2light.  Reconstructing the
    window from start_idx:stop_idx would therefore analyze the wrong time grid.
    """

    snapshot = payload.get("final_snapshot")
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("full"), dict):
        raise RuntimeError("AFM05 Step 3 requires final_snapshot.full in the st2l result payload")
    full = snapshot["full"]
    times = np.asarray(full["times"], dtype=float).reshape(-1)
    ode_window = np.asarray(full["ode_true"], dtype=float)
    if ode_window.shape != (3, times.size):
        raise ValueError(
            f"final_snapshot.full.ode_true must have shape (3, {times.size}), got {ode_window.shape}"
        )
    extra = {
        "x2dot": np.asarray(full["x2dot_true"], dtype=float).reshape(-1),
        "traj_pred": np.asarray(full["traj_pred"], dtype=float),
        "x2dot_pred": np.asarray(full["x2dot_pred"], dtype=float).reshape(-1),
        "force_pred": np.asarray(full["fts_rollout_pred"], dtype=float).reshape(-1),
    }
    if extra["x2dot"].size != times.size:
        raise ValueError("final_snapshot.full.x2dot_true length does not match times")
...
def observable_component_scales(ode_window: np.ndarray, extra: dict[str, np.ndarray]) -> dict[str, float]:
    scales = {
        "x1": rms(ode_window[0, :]),
        "x2": rms(ode_window[1, :]),
    }
...
def baseline_reproduction_metrics(
    outputs: dict[str, np.ndarray],
    snapshot_extra: dict[str, np.ndarray],
) -> dict[str, dict[str, float]]:
    references = {
        "x1": snapshot_extra["traj_pred"][0, :],
        "x2": snapshot_extra["traj_pred"][1, :],
        "x3_pred_diagnostic": snapshot_extra["traj_pred"][2, :],
        "x2dot": snapshot_extra["x2dot_pred"],
        "force_pred_diagnostic": snapshot_extra["force_pred"],
    }
    output_names = {
        "x1": "x1",
        "x2": "x2",
        "x3_pred_diagnostic": "x3",
        "x2dot": "x2dot",
        "force_pred_diagnostic": "force",
    }
    metrics: dict[str, dict[str, float]] = {}
    for label, reference in references.items():
        current = np.asarray(outputs[output_names[label]], dtype=float).reshape(-1)
        ref = np.asarray(reference, dtype=float).reshape(-1)
        if current.shape != ref.shape:
            raise ValueError(f"Baseline reproduction shape mismatch for {label}: {current.shape} vs {ref.shape}")
        diff = current - ref
        metrics[label] = {
            "relative_rms": float(raw_rms(diff) / max(raw_rms(ref), 1.0e-30)),
            "max_abs": float(np.max(np.abs(diff))) if diff.size else 0.0,
        }
    return metrics


def require_baseline_reproduction(metrics: dict[str, dict[str, float]], tolerance: float) -> None:
    failed = {
        name: values["relative_rms"]
        for name, values in metrics.items()
        if values["relative_rms"] > tolerance
    }
    if failed:
        details = ", ".join(f"{name}={value:.6e}" for name, value in failed.items())
        raise RuntimeError(
            "AFM05 Step 3 baseline does not reproduce the final st2l rollout; "
            f"relative-RMS tolerance={tolerance:.3e}, failed: {details}"
        )
...
def compute_mech_identifiability(
    *,
    ...
    times_np, ode_window, extra = final_snapshot_arrays(payload)
    ...
    reproduction = baseline_reproduction_metrics(baseline_outputs, extra)
    require_baseline_reproduction(reproduction, baseline_reproduction_tolerance)
    log_progress(
        "baseline reproduction passed | "
        + " ".join(f"{name}={values['relative_rms']:.3e}" for name, values in reproduction.items())
    )
    scales = observable_component_scales(ode_window, extra)
    for name in components:
    ...
    return {
        ...
        "baseline_reproduction": {
            "tolerance": float(baseline_reproduction_tolerance),
...
def compute_trained_identifiability(
    *,
    ...
    shard_count: int = 1,
    shard_index: int = 0,
    baseline_reproduction_tolerance: float = 1.0e-6,
) -> dict[str, Any]:
    model, known_pars, dtype = build_model(payload)
    mech0 = build_mech_from_state(payload, dtype)
    ks0, cs0 = mech_physical(mech0)
...
def write_outputs(result: dict[str, Any], results_dir: Path) -> dict[str, Path]:
    ...
    prefix = results_dir / f"afm05_step3_identifiability_{run_parameter_set}_{rank_label}_{stamp}"
    latest_prefix = results_dir / f"afm05_step3_identifiability_{run_parameter_set}_latest"
    ...
    lines = [
        "AFM05 step3 identifiability result",
        ...
        "baseline_reproduction: "
        + ", ".join(
            f"{name}={values['relative_rms']:.6e}"
            for name, values in result.get("baseline_reproduction", {}).get("metrics", {}).items()
        ),
        "",
...
def merge_trained_shards(shard_run_dir: Path, expected_shard_count: int | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "baseline_reproduction": first_meta.get("baseline_reproduction", {}),
        "sensitivity_column_rms": {
...
def main() -> None:
    ...
    parser.add_argument(
        "--entry",
        default="AFM05/step3_parameters_identifiability/results/current_step3_entry.json",
        help="Current step3 entry JSON written by set_afm05_step3_entry.py.",
    )
    parser.add_argument(
        "--results-dir",
        default="AFM05/step3_parameters_identifiability/results",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0e-6,
        help="Maximum relative-RMS mismatch allowed against the final st2l snapshot before finite differences run.",
    )
    parser.add_argument(
        "--components",
        default=",".join(DEFAULT_COMPONENTS),
        help="Comma-separated AFM05 observable components: x1,x2,x2dot.",
    )
    parser.add_argument(
        "--parameter-limit",
        type=int,
    ...
    if args.baseline_reproduction_tolerance <= 0.0:
        raise ValueError("--baseline-reproduction-tolerance must be positive")
    ...
    if args.merge_shards:
        ...
        print("AFM05 step3 shard merge completed")
    ...
    if args.parameter_set == "trained":
        result = compute_trained_identifiability(
            ...
            baseline_reproduction_tolerance=float(args.baseline_reproduction_tolerance),
        )
    else:
    ...
    if result.get("schema_version") == SHARD_SCHEMA:
        ...
        shard = result["shard"]
        print("AFM05 step3 sensitivity shard completed")
        print(f"object -> {result.get('rank_label')} | rank={result.get('rank')} | candidate B{result.get('candidate_b')}")
        print(
    ...
    print("AFM05 step3 identifiability completed")
    print(f"object -> {result.get('rank_label')} | rank={result.get('rank')} | candidate B{result.get('candidate_b')}")
    print(f"parameter_set -> {result['parameter_set']} | parameter_count={result['parameter_count']}")
