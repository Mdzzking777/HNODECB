"""
Select the current AFM05 stage2light local point for step3.

This script is the switchboard for AFM05 step3. It points step3 at one
...
from AFM05.step3_parameters_identifiability.afm05_stage2light_step3_entry import (  # noqa: E402
    as_repo_rel,
    build_manifest,
    jsonable,
    load_torch_payload,
...
DEFAULT_ARCHIVE_ROOT = "AFM05/Archive/st2l/valley"
DEFAULT_OUTPUT = "AFM05/step3_parameters_identifiability/results/current_step3_entry.json"
...
def summarize_candidate(rank_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if rank is None:
        rank = maybe_number(stage1.get("rank"))
    if rank is None:
    ...
    return {
        ...
        "source_mech_winner": maybe_number(warmstart.get("source_mech_winner")) or maybe_number(stage1.get("mech_winner")),
        "trial_id": maybe_number(warmstart.get("trial_id")) or maybe_number(resume_identity.get("prestage2_trial_id")) or maybe_number(stage1.get("trial_id")),
        "seed": maybe_number(warmstart.get("seed")) or maybe_number(stage1.get("seed")),
...
def completed_result_status(payload: dict[str, Any]) -> tuple[bool, list[str]]:
    missing: list[str] = []
    for key in ("final_state_dict", "final_mech_state_dict", "state_mean", "state_scale", "known_pars", "window_meta", "final_snapshot"):
...
def write_current_entry(selected: dict[str, Any], manifest_file: Path, output: Path) -> Path:
    ...
    current = {
        ...
        "purpose": "Current AFM05 step3 entry selection.",
        ...
        "warmstart_summary": {
            ...
            "rank": jsonable(warmstart.get("rank", warmstart.get("source_stage1_rank"))),
            "mech_winner": jsonable(warmstart.get("mech_winner", warmstart.get("source_mech_winner"))),
            "ks0": jsonable(warmstart.get("ks0")),
            "cs0": jsonable(warmstart.get("cs0")),
        },
        "note": "Replace the step3 object by rerunning set_afm05_step3_entry.py with another --rank/--candidate/--rank-dir.",
    }
...
def print_entries(entries: list[dict[str, Any]]) -> None:
    for e in entries:
        ...
        print(
            ...
            f"final_epoch={e.get('final_val_epoch')} "
