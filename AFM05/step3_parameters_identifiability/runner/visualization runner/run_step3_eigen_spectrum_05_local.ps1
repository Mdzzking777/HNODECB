...
$logPath = Join-Path $logsDir ("afm05_step3_visualization_eigen_spectrum_{0}.txt" -f $runStamp)
...
try {
  ...
  Write-Host "Running AFM05 step3 eigen-spectrum visualization"
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "AFM05 step3 eigen-spectrum visualization failed with exit code $LASTEXITCODE."
  }
} finally {
