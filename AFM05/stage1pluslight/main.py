"""CLI entrypoint for one AFM05 stage1pluslight shard."""
...
from AFM05.stage1pluslight.runner import Stage1Logger, make_shard_log_path, run_shard_mainloop


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run one AFM05 stage1pluslight shard.")
    parser.add_argument("--auto-generate-dataset", action="store_true", help="Generate AFM05 st1pl entry dataset if missing.")
    args = parser.parse_args(argv)

    config = default_config()
    log_path_env = os.environ.get("HNODECB_AFM05_STAGE1_LOG_PATH", "").strip()
