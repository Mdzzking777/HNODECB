...
$env:HNODECB_STAGE2_GRAD_SCALE_P_NET = "0.5"
...
$env:HNODECB_STAGE2_STEP_MAX_LOSS_INCREASE = "1e-8"
...
$env:HNODECB_STAGE2_LOG_SUBDIR = "stage2_step2a/local/windowed"
$env:HNODECB_STAGE2_LOG_PREFIX = "log2_03_step2a_stage2light_windowed_local"
...
if (-not (Test-Path -Path (Join-Path $repoRoot "logs\\stage2_step2a\\local\\windowed"))) {
  New-Item -ItemType Directory -Path (Join-Path $repoRoot "logs\\stage2_step2a\\local\\windowed") -Force | Out-Null
}

$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$stage2Log = Join-Path $repoRoot ("logs\\stage2_step2a\\local\\windowed\\log2_03_step2a_stage2light_windowed_local_driver_" + $timestamp + ".txt")
$stage2Runner = Join-Path $repoRoot "runner\\stagewise\\afm_param_search_stage2light_03.jl"

Write-Host "Running Stage2light WINDOWED (03) with JULIA_NUM_THREADS=$env:JULIA_NUM_THREADS, BLAS_THREADS=$env:OPENBLAS_NUM_THREADS, SELFTEST=$env:HNODECB_SELFTEST, SHARDS=$env:HNODECB_STAGE2_SHARD_COUNT, CANDIDATE=$env:HNODECB_STAGE2LIGHT_CANDIDATE, NN_RANK=$env:HNODECB_STAGE2_NN_WARM_RANK, INIT_RETRIES=$env:HNODECB_STAGE2_INIT_RETRIES, WINDOW_START=$env:HNODECB_STAGE2_WINDOW_START, WINDOW_LEN=$env:HNODECB_STAGE2_WINDOW_LEN"
Write-Host "Repo root  -> $repoRoot"
Write-Host "Project    -> $projectPath"
Write-Host "Driver log -> $stage2Log"
Write-Host "Shard logs -> logs\\stage2_step2a\\local\\windowed\\log2_03_step2a_stage2light_windowed_local_p#.txt"
