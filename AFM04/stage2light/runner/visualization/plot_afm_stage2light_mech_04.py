...
MECH_RE = re.compile(
    r"mech:\s*ks=([0-9eE+\-.]+)\s+\(([0-9eE+\-.]+)%\)\s+cs=([0-9eE+\-.]+)\s+\(([0-9eE+\-.]+)%\)"
)
...
def _annotation_epochs(epochs: list[int] | np.ndarray, step: int = 200) -> list[int]:
    epoch_list = [int(ep) for ep in epochs if np.isfinite(ep)]
    if not epoch_list:
        return []
    selected = [ep for ep in epoch_list if ep == 1 or ep % step == 0]
...
def parse_log_mech(log_path: Path) -> dict:
    ...
    for line in log_path.read_text(encoding="utf-8").splitlines():
        ...
        match = MECH_INIT_RE.search(line)
        if match is not None:
            truth_ks = float(match.group(3))
            truth_cs = float(match.group(4))
            continue

        match = MECH_RE.search(line)
        if match is not None and current_epoch is not None:
            ks_by_epoch[current_epoch] = float(match.group(1))
            cs_by_epoch[current_epoch] = float(match.group(3))
    ...
    return {
        ...
        "mech_true": np.asarray([truth_ks, truth_cs], dtype=float),
...
def run_one(*, log_dir: Path | None = None, out_dir: Path = DEFAULT_OUT_DIR) -> Path:
    ...
    for col, payload in enumerate(payloads):
        if payload.get("source") == "log":
            ...
            mech_true = np.asarray(payload["mech_true"], dtype=float)
            ...
            mech_true = np.asarray(result_payload.get("mech_true", [np.nan, np.nan]), dtype=float)
    ...
    return finalize_and_save(fig, out_path("afm_param_stage2light_04_mech_grid.png", out_dir))
