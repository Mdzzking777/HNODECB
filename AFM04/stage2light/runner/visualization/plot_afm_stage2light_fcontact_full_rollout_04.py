...
from AFM04.stage2light.rollout import LearnableMechModule, fts_truth_from_states_torch, rollout_single_shooting_torch
...
def main() -> None:
    ...
    mech_true_t = torch.as_tensor(np.asarray(payload["mech_true"], dtype=float), dtype=dtype, device="cpu")
    ...
    with torch.no_grad():
        ...
        rollout_states_pred = traj_pred_t.transpose(0, 1)
        rollout_states_true = torch.as_tensor(ode_rollout_true[:, eval_indices].T, dtype=dtype, device="cpu")
        ...
        fts_pred_t = model(rollout_states_pred)
        mech_pred = mech_module().detach().cpu().numpy().reshape(-1)

    fts_true_eval = fts_true_t.detach().cpu().numpy().reshape(-1)
    fts_pred_eval = fts_pred_t.detach().cpu().numpy().reshape(-1)
    ...
    for ax, (name, start, stop, indices) in zip(flat_axes, panels, strict=False):
        ...
        ax.plot(times_eval_us[pos], fts_true_eval[pos], color="black", linewidth=2.0, label="Fts teacher true", zorder=1)
        ax.plot(
            times_eval_us[pos],
            fts_pred_eval[pos],
            color="crimson",
            linewidth=2.0,
            linestyle="--",
            label="Fts rollout pred",
            zorder=3,
        )
        ax.set_title(_panel_title(name, start, stop, times_rollout))
        ax.set_xlabel("time (μs)")
        ax.set_ylabel("force (N)")
        ax.set_ylim(*_true_based_ylim(fts_true_eval[pos]))
    ...
    fig.suptitle(
        "AFM04 stage2light Fts postprocess rollout from W0 known IC\n"
    ...
    finalize_and_save(fig, OUT_DIR / "afm_param_stage2light_04_fcontact_full_rollout_grid.png")
