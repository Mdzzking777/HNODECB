"""Non-perturbed trajectory generation helpers for AFM04.

This module is the Python counterpart of
`datasets/non_perturbed_dataset_generator.jl`, but implemented with only
standard library + numpy so it works in the current local environment.
"""
...
def rk4_step(f: Callable[[float, np.ndarray], np.ndarray], t: float, u: np.ndarray, dt: float) -> np.ndarray:
    """Classical RK4 step for a fixed-step trajectory rollout."""
...
def build_table(column_names: Sequence[str], matrix: np.ndarray) -> dict[str, np.ndarray]:
    """Convert a 2D matrix into a dict-of-columns table."""
...
def table_to_matrix(table: Mapping[str, Any], column_names: Sequence[str]) -> np.ndarray:
    """Convert a dict-of-columns table back to a 2D matrix."""
...
def save_table_npz(path: str, table: Mapping[str, Any], column_names: Sequence[str]) -> None:
    """Persist a dict-of-columns table in a Python-native portable format."""
...
def load_table_npz(path: str) -> dict[str, np.ndarray]:
    """Load a dict-of-columns table saved by ``save_table_npz``."""
...
def generate_non_perturbed_training_set(
    ...
    *,
    ...
    """Roll out a noise-free trajectory and return it as a dict-of-columns table."""
