"""Post-analysis for AFM04 step3 identifiability results.

This script reads an already-computed AFM04 step3 result JSON and performs the
author-style null-space projection analysis:

  - eigenvalue spectrum with AFM04 null threshold
...
RESULT_SCHEMA_PREFIX = "afm04_step3_identifiability"
...
def write_text_summary(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append("AFM04 step3 post-analysis")
    lines.append("==========================")
    lines.append("")
...
def plot_eigen_spectrum(
    ...
    *,
    ...
    ax.set_title(f"AFM04 Step3 Eigen Spectrum ({rank_label})")
...
def plot_projection_groups(
    ...
    *,
    ...
    for ax, tau in zip(axes_flat, PROJECTION_GROUP_TAUS):
...
def plot_threshold_sweep(
    ...
    *,
    ...
    ax.set_title(f"AFM04 Step3 Null Projection Threshold Sweep ({rank_label})")
...
def main() -> int:
    ...
    parser.add_argument(
        "--input-result",
        type=Path,
        default=THIS_DIR / "results" / "afm04_step3_identifiability_trained_latest.json",
    ...
    base = f"afm04_step3_post_analysis_{tag}"
    ...
    json_payload = {
        "schema_version": "afm04_step3_post_analysis_v1",
    ...
    if not args.no_latest:
        latest_base = "afm04_step3_post_analysis_latest"
