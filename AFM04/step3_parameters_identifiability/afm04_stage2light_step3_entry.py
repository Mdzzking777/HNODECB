"""
AFM04 stage2light archive entry helper for step3 identifiability.

The original HNODECB step3 scripts read Julia .jld local-optimum payloads.
AFM04 stage2light archives are PyTorch payloads, so this helper defines a
...
point explicit and reproducible for the AFM04 step3 implementation.
"""
...
SCHEMA_VERSION = "afm04_stage2light_step3_entry_v1"


RANK_CANDIDATE_RE = re.compile(r"rank(?P<rank>\d+)(?:[_-]?B(?P<candidate>\d+))?", re.IGNORECASE)
...
class EntryPaths:
    ...
    def from_rank_dir(cls, rank_dir: Path) -> "EntryPaths":
        return cls(
            ...
            best_checkpoint=rank_dir / "checkpoint" / "stage2light_best_p1.pt",
            best_checkpoint_viz=rank_dir / "checkpoint" / "stage2light_best_p1.viz.pkl",
            resume_checkpoint=rank_dir / "checkpoint" / "stage2light_checkpoint_p1.pt",
...
def load_torch_payload(path: Path) -> dict[str, Any]:
    # PyTorch 2.6 defaults weights_only=True; AFM04 payloads contain metadata.
...
def parse_rank_candidate_label(label: str) -> tuple[int | None, int | None]:
    match = RANK_CANDIDATE_RE.search(str(label))
    if not match:
        return None, None
...
def payload_candidate_meta(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "original_rank": jsonable(warmstart.get("original_rank", warmstart.get("source_stage1_rank"))),
        "source_stage1_rank": jsonable(warmstart.get("source_stage1_rank")),
        "source_mech_winner": jsonable(warmstart.get("source_mech_winner")),
        "trial_id": jsonable(warmstart.get("trial_id", resume_identity.get("prestage2_trial_id"))),
        "seed": jsonable(warmstart.get("seed")),
        "label": jsonable(warmstart.get("label")),
...
def payload_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    summary["known_pars"] = jsonable(payload.get("known_pars", {}))
    summary["mech_true"] = jsonable(payload.get("mech_true", {}))
...
def build_manifest(rank_dir: Path, entry_payload_kind: str = "result") -> dict[str, Any]:
    ...
    best_payload = load_torch_payload(paths.best_checkpoint)
    checkpoint_payload = load_torch_payload(paths.resume_checkpoint)

    path_map = {
        "result": paths.result_payload,
        "best": paths.best_checkpoint,
        "checkpoint": paths.resume_checkpoint,
    }
    if entry_payload_kind not in path_map:
        raise ValueError(f"entry_payload_kind must be one of {sorted(path_map)}, got {entry_payload_kind!r}")
    ...
    best_mech = best_payload.get("mech_state_dict") or best_payload.get("best_snapshot", {}).get("mech_state_dict")
    checkpoint_mech = checkpoint_payload.get("mech_state_dict")

    return {
        "schema_version": SCHEMA_VERSION,
        "source_project": "AFM04",
        ...
        "entry_payload": as_repo_rel(path_map[entry_payload_kind]),
        "paths": {
            "result_payload": as_repo_rel(paths.result_payload),
            "result_viz_payload": as_repo_rel(paths.result_viz_payload),
            "best_checkpoint": as_repo_rel(paths.best_checkpoint),
            "best_checkpoint_viz": as_repo_rel(paths.best_checkpoint_viz),
            "resume_checkpoint": as_repo_rel(paths.resume_checkpoint),
            "ode_data": as_repo_rel(paths.ode_data),
            "pert_df": as_repo_rel(paths.pert_df),
        },
        "result_summary": payload_summary(result_payload),
        "best_summary": payload_summary(best_payload),
        "checkpoint_summary": payload_summary(checkpoint_payload),
        "mechanistic_parameters": {
            "final": compact_state_dict(final_mech),
            "best": compact_state_dict(best_mech),
            "checkpoint": compact_state_dict(checkpoint_mech),
        },
        "normalizer": {
        ...
        "step3_notes": [
            "Use result payload for the final local point.",
            "Use best checkpoint if step3 should analyze the best validation point.",
...
def validate_manifest(path: Path) -> dict[str, Any]:
    ...
    required_keys = ["state_mean", "state_scale", "known_pars", "window_meta"]
    if manifest.get("entry_payload_kind") == "result":
        required_keys.extend(["final_state_dict", "final_mech_state_dict"])
    else:
        required_keys.extend(["state_dict", "mech_state_dict"])
...
def main() -> None:
    ...
    parser.add_argument(
        "--archive-root",
        default=r"AFM04/archive/st2l/3_10x10x200_W0 new",
