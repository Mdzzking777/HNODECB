...
$selectorScript = Join-Path $step3Root "set_afm04_step3_entry.py"
$computeScript = Join-Path $step3Root "compute_afm04_step3_identifiability.py"
...
$logPath = Join-Path $logsDir ("afm04_step3_identifiability_{0}.txt" -f $runStamp)

if (-not (Test-Path -LiteralPath $selectorScript)) {
  throw "Missing AFM04 step3 selector script: $selectorScript"
}
if (-not (Test-Path -LiteralPath $computeScript)) {
  throw "Missing AFM04 step3 compute script: $computeScript"
}
...
try {
  ...
  Write-Host "Running AFM04 step3 entry selector"
  Write-Host "Repo root -> $repoRoot"
  Write-Host "Python    -> $pythonExe"
  Write-Host "Archive   -> $ArchiveRoot"
  Write-Host "Payload   -> $EntryPayloadKind"
  Write-Host "Step3     -> parameter_set=$ParameterSet finite_diff_eps=$FiniteDiffEps components=$Components parameter_limit=$ParameterLimit shard_count=$ShardCount"
  ...
  if ($exitCode -ne 0) {
    throw "AFM04 step3 entry selector failed with exit code $exitCode."
  }
  ...
  if (-not $List.IsPresent) {
    ...
    if ($SkipIdentifiability.IsPresent) {
      ...
      if (($ParameterSet -eq "trained") -and ($ShardCount -gt 1)) {
        ...
        Write-Host "Running AFM04 step3 trained parameter-column shards"
        Write-Host "Shard run -> $shardRunDir"

        $jobs = @()
        for ($shardIndex = 1; $shardIndex -le $ShardCount; $shardIndex++) {
          $shardLog = Join-Path $logsDir ("afm04_step3_identifiability_{0}_shard_{1:D3}_of_{2:D3}.txt" -f $runStamp, $shardIndex, $ShardCount)
          $shardArgs = @(
            $computeScript,
            "--entry", $currentEntry,
        ...
        if ($failedJobs.Count -gt 0) {
          throw "One or more AFM04 step3 shard jobs failed: $($failedJobs.Count)"
        }
        ...
        if ($computeExitCode -ne 0) {
          throw "AFM04 step3 shard merge/eigen computation failed with exit code $computeExitCode."
        }
      } else {
        ...
        Write-Host "Running AFM04 step3 identifiability eigen computation"
        & $pythonExe @computeArgs 2>&1 | ForEach-Object { Write-Host $_ }
        $computeExitCode = $LASTEXITCODE
        if ($computeExitCode -ne 0) {
          throw "AFM04 step3 identifiability computation failed with exit code $computeExitCode."
        }
      }
