...
$analysisScript = Join-Path $step3Root "post_analyze_afm04_step3_identifiability.py"
$logPath = Join-Path $logsDir ("afm04_step3_post_analysis_{0}.txt" -f $runStamp)

if (-not (Test-Path -LiteralPath $analysisScript)) {
  throw "Missing AFM04 step3 post-analysis script: $analysisScript"
}
...
try {
  ...
  Write-Host "Running AFM04 step3 post-analysis"
  ...
  if ($exitCode -ne 0) {
    throw "AFM04 step3 post-analysis failed with exit code $exitCode."
  }
