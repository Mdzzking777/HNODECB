...
$logPath = Join-Path $logsDir ("afm04_step3_visualization_null_projection_groups_{0}.txt" -f $runStamp)
...
try {
  ...
  Write-Host "Running AFM04 step3 null-projection groups visualization"
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "AFM04 step3 null-projection groups visualization failed with exit code $LASTEXITCODE."
  }
} finally {
