"""Training loop for the isolated AFM04 MLP full functional test."""
...
from AFM04.MLP_full_test.data import PreparedData, WindowSplit, prepare_data
from AFM04.MLP_full_test.mlp_backend import MLPForceModule, summarize_wpred_from_states
from AFM04.MLP_full_test.losses import TorchLossParts, evaluate_split
from AFM04.MLP_full_test.rollout import fts_truth_from_states_torch, x2dot_rhs_torch
...
def _run_oracle_sanity(
    *,
    ...
    _log_line(log, "=== MLP FULL TEST SANITY (oracle F_contact + true mech) ===")
...
def _maybe_load_random_search_warmstart(
    *,
    ...
    best_path = cfg.random_search_checkpoint_dir / f"MLP_full_test_random_search_best_{tag}.pt"
...
def run_full_test_shard(cfg=None, *, shard_index: int | None = None, log_path: Path | None = None) -> dict[str, object]:
    ...
    history_path = cfg.result_dir / f"MLP_full_test_history_{file_tag}.json"
    checkpoint_path = cfg.checkpoint_dir / f"MLP_full_test_best_{file_tag}.pt"
    periodic_checkpoint_path = cfg.checkpoint_dir / f"MLP_full_test_checkpoint_{file_tag}.pt"
    result_path = cfg.result_dir / f"MLP_full_test_result_{file_tag}.pt"
    viz_result_path = cfg.result_dir / f"MLP_full_test_result_{file_tag}.viz.pkl"
    log_path = cfg.shard_log_dir / f"log2_04_step2a_MLP_full_test_local_{file_tag}.txt" if log_path is None else Path(log_path)
    ...
    with log_path.open("w", encoding="utf-8") as log:
        _log_line(
            log,
            "MLP full test windowed horizon: "
        ...
        _log_line(
            log,
            f"MLP shard assignment: {shard_index}/{cfg.shard_count} | local_windows=1 | "
        ...
        for epoch in range(cfg.epochs):
            ...
            if ((epoch + 1) % cfg.log_every) == 0 or epoch == 0 or epoch + 1 == cfg.epochs:
                _log_line(log, f"MLP epoch {epoch + 1} train={float(train_total.detach()):.6e}")
                ...
                _log_line(log, f"MLP val epoch {epoch + 1} val={float(val_total.detach()):.6e}")
                _log_line(
                    log,
                    "  val parts: "
        ...
        if failure_reason != "":
            _log_line(log, f"MLP shard failure -- epoch={failure_epoch} reason={failure_reason}")
        ...
        _log_line(
            log,
            f"MLP shard done -- train={history[-1]['train_loss']:.6e} val={history[-1]['val_loss']:.6e} "
...
def merge_shard_results(cfg=None) -> dict[str, Any]:
    ...
    for shard_index in range(1, cfg.shard_count + 1):
        window_index = int(selected_window_indices[shard_index - 1])
        split = prepared.splits[window_index - 1]
        result_path = cfg.result_dir / f"MLP_full_test_result_{_window_file_tag(split.role)}.pt"
    ...
    summary_path = cfg.result_dir / "MLP_full_test_merged_summary.json"
