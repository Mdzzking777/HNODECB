"""CLI entrypoint for the isolated AFM04 MLP full functional test."""
...
from AFM04.MLP_full_test.random_search import (
    merge_random_search_results,
    run_random_search_shard,
    write_random_search_seed_bank,
)
from AFM04.MLP_full_test.train import merge_shard_results, run_full_test_shard
...
def _driver_log_path(cfg) -> Path:
    ...
    return cfg.log_dir / f"log2_04_step2a_MLP_full_test_local_driver_{stamp}.txt"


def _shard_log_path(cfg, shard_index: int) -> Path:
    return cfg.shard_log_dir / "log2_04_step2a_MLP_full_test_local_w0.txt"


def _random_search_driver_log_path(cfg) -> Path:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return cfg.random_search_log_dir / f"log2_04_step2a_MLP_full_test_random_search_local_driver_{stamp}.txt"


def _random_search_shard_log_path(cfg, subshard_index: int) -> Path:
    return cfg.random_search_shard_log_dir / f"log2_04_step2a_MLP_full_test_random_search_local_s{subshard_index}.txt"
...
def run_driver(
    ...
    *,
    ...
    base_env["HNODECB_AFM04_MLP_TEST_SHARD_COUNT"] = str(cfg.shard_count)
    base_env["HNODECB_AFM04_MLP_TEST_WARMSTART_FROM_RANDOM_SEARCH"] = "1" if warmstart_from_random_search else "0"
    base_env["HNODECB_AFM04_MLP_TEST_WARMSTART_FALLBACK_RANDOM"] = "1" if warmstart_fallback_random else "0"
    if auto_generate_dataset:
        base_env["HNODECB_AFM04_MLP_TEST_AUTOGEN_DATASET"] = "1"

    with driver_log.open("w", encoding="utf-8") as log:
        _log_line(log, f"AFM04 MLP full test driver start | shard_count={cfg.shard_count}")
        ...
        for shard_index in range(1, cfg.shard_count + 1):
            ...
            env = dict(base_env)
            env["HNODECB_AFM04_MLP_TEST_SHARD_INDEX"] = str(shard_index)
            proc = subprocess.Popen(
                [sys.executable, "-m", "AFM04.MLP_full_test.main", "--shard-index", str(shard_index)],
        ...
        if failed:
            ...
            raise RuntimeError(f"AFM04 MLP full test driver failed: {failed}")
...
def run_random_search_driver(cfg, *, auto_generate_dataset: bool) -> dict[str, object]:
    ...
    base_env = dict(os.environ)
    base_env["HNODECB_AFM04_MLP_TEST_SHARD_COUNT"] = str(cfg.shard_count)
    base_env["HNODECB_AFM04_MLP_TEST_WARMSTART_FROM_RANDOM_SEARCH"] = "0"
    base_env["HNODECB_AFM04_MLP_TEST_RANDOM_SEARCH_SUBSHARD_COUNT"] = str(cfg.random_search_subshard_count)
    base_env["HNODECB_AFM04_MLP_TEST_RANDOM_SEARCH_WINDOW_INDEX"] = str(cfg.random_search_window_index)
    if auto_generate_dataset:
        base_env["HNODECB_AFM04_MLP_TEST_AUTOGEN_DATASET"] = "1"
    ...
    with driver_log.open("w", encoding="utf-8") as log:
        ...
        selected_window_text = (
            "W0"
            if str(cfg.window_mode).strip().lower() in ("w0", "stage2_w0", "stage2-w0", "first_contact", "first-contact")
            else (f"W{cfg.random_search_window_index}" if cfg.random_search_window_index > 0 else f"{cfg.shard_count} selected windows")
        )
        _log_line(
            log,
            "AFM04 MLP random-search driver start | "
        ...
        for subshard_index in range(1, cfg.random_search_subshard_count + 1):
            ...
            env = dict(base_env)
            env["HNODECB_AFM04_MLP_TEST_RANDOM_SEARCH_SUBSHARD_INDEX"] = str(subshard_index)
            proc = subprocess.Popen(
                [
                    sys.executable,
                    "-m",
                    "AFM04.MLP_full_test.main",
                    "--random-search-shard",
        ...
        if failed:
            ...
            raise RuntimeError(f"AFM04 MLP random-search driver failed: {failed}")
