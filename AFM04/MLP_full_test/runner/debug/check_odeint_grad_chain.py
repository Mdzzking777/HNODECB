"""Check whether rollout gradients truly flow back through odeint to MLP params."""
...
from AFM04.MLP_full_test.losses import (
    CONTACT_LOSS_WEIGHT,
...
from AFM04.MLP_full_test.rollout import rollout_single_shooting_torch
...
def _iter_named_params(model: MLPForceModule):
    wanted = (
        "log_gnn",
        "mlp.0.weight",
        "mlp.0.bias",
...
def _grad_summary(model: MLPForceModule) -> dict[str, dict[str, float | bool | None]]:
...
def _zero_grads(model: MLPForceModule) -> None:
...
def run_check(*, shard_index: int, max_points: int, seed: int | None = None) -> dict[str, Any]:
    ...
    # 1. Full training loss path used by MFT.
    _zero_grads(model)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check whether MFT rollout gradients truly pass through odeint.")
