...
Write-Host "Running MFT AD-vs-FD check"
Write-Host "Repo root   -> $repoRoot"
Write-Host "Python      -> $pythonExe"
Write-Host "Shard index -> $ShardIndex"
Write-Host "Max points  -> $MaxPoints"

& $pythonExe -m AFM04.MLP_full_test.runner.debug.check_ad_vs_fd --shard-index $ShardIndex --max-points $MaxPoints
$exitCode = $LASTEXITCODE

if ($exitCode -ne 0) {
  throw "MFT AD-vs-FD check failed with exit code $exitCode"
}
