...
SCRIPT_ORDER = [
    ...
    ("plot_afm_mlp_full_test_recon_nn_04.py", "log"),
]
...
def main() -> None:
    ...
    print(f"[inputs] logs={'yes' if has_logs else 'no'}")
    ...
    for script_name, mode in SCRIPT_ORDER:
        ...
        if mode == "log" and not has_logs:
            print(f"[skip] {script_name} -- missing log inputs")
            continue
    ...
    if ran_any:
        print("[done] AFM04 MLP full-test core visualizations generated")
    else:
        print("[done] no AFM04 MLP full-test log inputs found; nothing generated")
