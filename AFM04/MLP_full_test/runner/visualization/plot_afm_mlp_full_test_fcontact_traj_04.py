...
def main() -> None:
    ...
    for ax, payload in zip(axes[0], payloads):
        ...
        ax.plot(times_us, f_true, color="black", linewidth=2, label="Fts teacher true")
        ax.plot(times_us, f_pred, color="crimson", linewidth=2, linestyle="--", label="Fts rollout pred")
        ax.set_title(title)
        ax.set_xlabel("time (μs)")
        ax.set_ylabel("force (N)")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="best")
    finalize_and_save(fig, out_path("afm_param_MLP_full_test_04_fcontact_grid.png"))
