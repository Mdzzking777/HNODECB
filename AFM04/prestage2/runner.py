"""Runner helpers for AFM04 prestage2 (prest2)."""
...
from AFM04.prestage2.config import Prestage2Config, default_config
from AFM04.stage2light.config import default_config as default_stage2light_config
from AFM04.stage2light.data import load_stage1_payload, prepare_data
...
def make_shard_log_path(log_dir: Path, layer_name: str, shard_index: int) -> Path:
    ...
    return log_dir / f"log2_04_prest2_local_{token}_p{shard_index}.txt"
...
def _build_mech_winner_items(stage1_payload: dict[str, Any], cfg: Prestage2Config) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "ks": float(first.get("ks_hat", first_params.get("ks0", float("nan")))),
                "cs": float(first.get("cs_hat", first_params.get("cs0", float("nan")))),
                "ks_err_pct": float(first.get("ks_err_pct", float("nan"))),
                "cs_err_pct": float(first.get("cs_err_pct", float("nan"))),
                "n_total": int(n_total),
    ...
    if cfg.layer_a_top_candidates > len(rows):
        raise ValueError(
            f"Requested {cfg.layer_a_top_candidates} mech winners for Layer A, "
            f"but only {len(rows)} mechanical groups exist"
        )
    return rows[: int(cfg.layer_a_top_candidates)]


def _stage2light_child_log_path(candidate_root: Path) -> Path:
    return candidate_root / "logs" / "window_per_shard" / "log2_04_step2a_stage2light_local_p1.txt"
...
def _layer_shard_result_path(cfg: Prestage2Config, layer_name: str, shard_index: int) -> Path:
    token = _layer_token(layer_name)
    return cfg.result_dir / f"afm_prest2_04_{token}_p{shard_index}.pkl"


def _layer_assignment_path(cfg: Prestage2Config, layer_name: str, shard_index: int) -> Path:
    token = _layer_token(layer_name)
    return cfg.result_dir / f"afm_prest2_04_{token}_assignments_p{shard_index}.json"
...
def _layer_merged_result_path(cfg: Prestage2Config, layer_name: str) -> Path:
    ...
    if canonical == LAYER_A:
        return cfg.result_dir / "afm_prest2_04_top_mech_winners_a.pkl"
    return cfg.result_dir / "afm_prest2_04_candidates_b.pkl"


def _legacy_layer_merged_result_path(cfg: Prestage2Config, layer_name: str) -> Path:
    canonical = _canonical_layer_name(layer_name)
    if canonical == LAYER_A:
        return cfg.result_dir / "afm_prest2_04_newrank_a.pkl"
    return cfg.result_dir / "afm_prest2_04.pkl"
...
def _extract_candidate_record(
    *,
    ...
    record = {
        ...
        "ks_err_pct": float(final_row.get("ks_err_pct", float("nan"))),
        "cs_err_pct": float(final_row.get("cs_err_pct", float("nan"))),
        "train_x1_rec": float(final_row.get("train_x1_rec", final_row.get("x1_rec", float("nan")))),
        "train_x3_rec": float(final_row.get("train_x3_rec", final_row.get("x3_rec", float("nan")))),
        "val_x1_rec": float(final_row.get("val_x1_rec", float("nan"))),
        "val_x3_rec": float(final_row.get("val_x3_rec", float("nan"))),
        "train_nn_err": float(final_row.get("train_fts_teacher_rec", final_row.get("fts_teacher_rec", float("nan")))),
        "val_nn_err": float(final_row.get("val_fts_teacher_rec", float("nan"))),
...
def run_prestage2_shard(
    ...
    *,
    ...
    with shard_log_path.open("w", encoding="utf-8") as log:
        item_summary = [
            (
        ...
        for loop_idx, item in enumerate(items, start=1):
            ...
            if canonical_layer == LAYER_B:
                ...
                for stale_path in (
                    candidate_root / "results" / "stage2light_result_p1.pt",
                    candidate_root / "results" / "stage2light_result_p1.viz.pkl",
                ):
            ...
            for key in (
                ...
                "HNODECB_AFM04_STAGE2LIGHT_INPUT_TRIAL_ID",
                "HNODECB_AFM04_STAGE2LIGHT_INPUT_MECH_WINNER",
                "HNODECB_AFM04_STAGE2LIGHT_INPUT_CANDIDATE",
                "HNODECB_AFM04_STAGE2LIGHT_PREST2_INPUT_PATH",
                "HNODECB_AFM04_STAGE2LIGHT_WARMSTART_FROM_STAGE1",
            ):
                env.pop(key, None)
            env["HNODECB_AFM04_STAGE2LIGHT_OUTPUT_ROOT"] = str(candidate_root)
            env["HNODECB_AFM04_STAGE2LIGHT_SHARD_COUNT"] = "1"
            env["HNODECB_AFM04_STAGE2LIGHT_AUTOGEN_DATASET"] = "1" if cfg.auto_generate_dataset else "0"
            stage2_cfg = default_stage2light_config(cfg.repo_root)
            ...
            env["HNODECB_AFM04_STAGE2LIGHT_EPOCH_RETRIES"] = "8"
            ...
            env["HNODECB_AFM04_STAGE2LIGHT_STEP_RETRIES"] = "6"
            ...
            if canonical_layer == LAYER_A:
                ...
                env["HNODECB_AFM04_STAGE2LIGHT_INPUT_TRIAL_ID"] = str(source_stage1_trial_id)
                env["HNODECB_AFM04_STAGE2LIGHT_INPUT_MECH_WINNER"] = str(source_mech_winner)
                launch_msg = (
            ...
            _log_line(
                ...
                f"ks={record['ks_hat']:.6e} ({record['ks_err_pct']:.2f}%) | "
                f"cs={record['cs_hat']:.6e} ({record['cs_err_pct']:.2f}%) | "
                f"val_x1={record['val_x1_rec']:.2f}% | "
                f"val_x3={record['val_x3_rec']:.2f}% | "
                f"val_nn={record['val_nn_err']:.2f}% | "
                f"{_retry_summary_line(retry_summary)}",
            )

        _log_line(
            log,
            f"AFM04 prestage2 shard done | layer={canonical_layer} | "
...
def _run_layer_processes(
    ...
    *,
    ...
    base_env["HNODECB_AFM04_PREST2_SHARD_COUNT"] = str(cfg.shard_count)
    base_env["HNODECB_AFM04_PREST2_CHECKPOINT_EVERY"] = str(cfg.checkpoint_every)
    base_env["HNODECB_AFM04_PREST2_INPUT_PATH"] = str(cfg.stage1_input_path)
    base_env["HNODECB_AFM04_PREST2_OUTPUT_ROOT"] = str(cfg.output_root)
    if cfg.auto_generate_dataset:
        base_env["HNODECB_AFM04_PREST2_AUTOGEN_DATASET"] = "1"

    for shard_index in range(1, cfg.shard_count + 1):
        env = dict(base_env)
        env["HNODECB_AFM04_PREST2_SHARD_INDEX"] = str(shard_index)
        args = [
            sys.executable,
            "-m",
            "AFM04.prestage2.main",
            "--shard-index",
            str(shard_index),
            "--layer",
            canonical_layer,
        ...
        _log_line(
            driver_log,
            f"launched {canonical_layer} shard {shard_index}/{cfg.shard_count} | pid={proc.pid} | log={shard_log}",
            echo=True,
        )
    ...
    if failed:
        raise RuntimeError(f"AFM04 prestage2 {canonical_layer} failed: {failed}")
    ...
    if canonical_layer == LAYER_A:
        ...
        for record in records:
            _log_line(
                ...
                f"ks={float(record.get('ks_hat', float('nan'))):.6e} ({float(record.get('ks_err_pct', float('nan'))):.2f}%) | "
                f"cs={float(record.get('cs_hat', float('nan'))):.6e} ({float(record.get('cs_err_pct', float('nan'))):.2f}%) | "
                f"val_x1={float(record.get('val_x1_rec', float('nan'))):.2f}% | "
                f"val_x3={float(record.get('val_x3_rec', float('nan'))):.2f}% | "
                f"val_nn={float(record.get('val_nn_err', float('nan'))):.2f}%",
                echo=True,
            )
        return
