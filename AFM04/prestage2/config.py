"""Configuration for AFM04 prestage2 (prest2) trial-run screening."""
...
def default_config(repo_root: str | Path | None = None) -> Prestage2Config:
    ...
    output_root_raw = os.environ.get("HNODECB_AFM04_PREST2_OUTPUT_ROOT", "").strip()
    output_root = Path(output_root_raw).resolve() if output_root_raw != "" else (repo_root / "AFM04" / "prestage2")
    ...
    stage1_input_raw = os.environ.get("HNODECB_AFM04_PREST2_INPUT_PATH", "").strip()
    stage1_input_path = Path(stage1_input_raw).resolve() if stage1_input_raw != "" else (
        repo_root / "AFM04" / "stage1pluslight" / "results_afm" / "afm_param_stage1pluslight_04.pkl"
    )
    ...
    return Prestage2Config(
        ...
        shard_index=max(1, _env_int("HNODECB_AFM04_PREST2_SHARD_INDEX", 1)),
        shard_count=max(1, _env_int("HNODECB_AFM04_PREST2_SHARD_COUNT", 8)),
        layer_a_top_candidates=max(1, _env_int("HNODECB_AFM04_PREST2_LAYER_A_TOP_CANDIDATES", 100)),
        layer_a_epochs=max(1, _env_int("HNODECB_AFM04_PREST2_LAYER_A_EPOCHS", 10)),
        layer_b_top_candidates=max(1, _env_int("HNODECB_AFM04_PREST2_LAYER_B_TOP_CANDIDATES", 20)),
        layer_b_epochs=max(1, _env_int("HNODECB_AFM04_PREST2_LAYER_B_EPOCHS", 20)),
        checkpoint_every=max(1, _env_int("HNODECB_AFM04_PREST2_CHECKPOINT_EVERY", 5)),
        auto_generate_dataset=_env_bool("HNODECB_AFM04_PREST2_AUTOGEN_DATASET", False),
        candidate_metric=(
