...
def main() -> None:
    ...
    for ax, (name, start, stop) in zip(flat_axes, ctx.panels, strict=False):
        ...
        ax.plot(
            ...
            ctx.ode_rollout_true[0, panel_plot_pos],
            color="black",
            linewidth=2.0,
            label="x1 true",
            zorder=1,
        ...
        ax.plot(
            ctx.times_us[panel_plot_pos],
            ctx.traj_pred[0, panel_plot_pos],
            color="crimson",
            linewidth=1.5,
            linestyle="-",
            label="x1 pred",
    ...
    fig.suptitle(figure_title(ctx, "x1"), fontsize=14, y=0.98)
    finalize_and_save(fig, OUT_DIR / "afm_param_kan_full_test_04_x1_full_rollout_grid.png")
