"""Small-scope AD-vs-FD sanity check for KFT without interrupting GS."""
...
def _selected_param_specs(model: torch.nn.Module) -> list[tuple[str, torch.nn.Parameter, int]]:
    ...
    wanted = [
        ("log_gnn[0]", "log_gnn", 0),
        ("act0.coef[0]", "kan.act_fun.0.coef", 0),
        ("act0.scale_base[0]", "kan.act_fun.0.scale_base", 0),
        ("act0.scale_sp[0]", "kan.act_fun.0.scale_sp", 0),
        ("act1.coef[0]", "kan.act_fun.1.coef", 0),
        ("act1.scale_base[0]", "kan.act_fun.1.scale_base", 0),
        ("act1.scale_sp[0]", "kan.act_fun.1.scale_sp", 0),
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Small-scope AD-vs-FD check for AFM04 KFT.")
