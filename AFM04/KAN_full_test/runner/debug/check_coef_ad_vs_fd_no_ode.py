"""Check coef AD-vs-FD on direct KAN(states_true) teacher loss, bypassing odeint."""
...
def _teacher_loss(model: KANForceModule, states_true: torch.Tensor, fts_true: torch.Tensor) -> torch.Tensor:
...
def run_check(*, shard_index: int, max_points: int, rel_eps: float, abs_eps: float, seed: int | None = None) -> dict:
    ...
    param_specs = [
        ("kan.act_fun.0.coef", [0, 1, 2, 5, 10, 20, 40, 80, 119]),
        ("kan.act_fun.1.coef", [0, 1, 2, 5, 10, 20, 39]),
    ]
    checks = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Direct KAN teacher-loss coef AD-vs-FD check without odeint.")
