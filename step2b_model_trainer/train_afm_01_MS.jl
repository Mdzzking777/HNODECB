...
println("Sanity check loss (data-based, post-contact): ", sanity_data, " | state=", sanity_data_comps.state, " x3=", sanity_data_comps.x3, " (u0=", sanity_data_comps.x3_u0, ") x2dot=", sanity_data_comps.x2dot, " cont=", sanity_data_comps.continuity)
println("Sanity check loss (integrated, post-contact): ", sanity_integrated, " | state=", sanity_integrated_comps.state, " x3=", sanity_integrated_comps.x3, " (u0=", sanity_integrated_comps.x3_u0, ") x2dot=", sanity_integrated_comps.x2dot, " cont=", sanity_integrated_comps.continuity)
...
println("Sanity check loss (integrated, multiple shooting: oracle): ", sanity_integrated_ms_oracle, " | state=", sanity_integrated_ms_oracle_comps.state, " x3=", sanity_integrated_ms_oracle_comps.x3, " (u0=", sanity_integrated_ms_oracle_comps.x3_u0, ") x2dot=", sanity_integrated_ms_oracle_comps.x2dot, " cont=", sanity_integrated_ms_oracle_comps.continuity)
println("Sanity check loss (integrated, multiple shooting: model): ", sanity_integrated_ms_model, " | state=", sanity_integrated_ms_model_comps.state, " x3=", sanity_integrated_ms_model_comps.x3, " (u0=", sanity_integrated_ms_model_comps.x3_u0, ") x2dot=", sanity_integrated_ms_model_comps.x2dot, " cont=", sanity_integrated_ms_model_comps.continuity)
...
for run_id in 1:length(initial_guesses)
  ...
  println("Run ", run_id, " init diagnostics (", guess.source, "): loss=", @sprintf("%.4e", init_loss),
    ...
    " x3=", @sprintf("%.4e", init_comps.x3), " (u0=", @sprintf("%.4e", init_comps.x3_u0), ")",
  ...
  function callback(θ, l)
    ...
    if (stage_ref[] == "adam" && epoch % print_interval_adam == 0) ||
      ...
      println("Run ", run_id, " ", stage_ref[], " Epoch ", epoch, " -- cost: ", l,
        " | state=", comps.state, " x3=", comps.x3, " (u0=", comps.x3_u0, ") x2dot=", comps.x2dot, " cont=", comps.continuity,
