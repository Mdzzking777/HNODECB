...
function state_scale_pair(state12_scale)
  return state12_scale[1], state12_scale[2]
end
...
function loss_single_ss_nomonitor(θ, ode_data, x2dot_data, contact_mask, times,
  ...
  re_pnet, ks_bounds, cs_bounds; nn_gain::Float64=1.0)
  ...
  (string(sol.retcode) == "Success" && size(sol, 2) == length(times)) || return Inf
  uhat = Array(sol)

  x1_err = vec(abs2.((ode_data[1, :] .- uhat[1, :]) ./ x1_state_scale))
  x2_err = vec(abs2.((ode_data[2, :] .- uhat[2, :]) ./ x2_state_scale))
  total_state = sum(weights_val .* x1_err) / weights_sum + sum(weights_val .* x2_err) / weights_sum
...
function fd_dir(loss_fn, θ, h, p_dir::Vector{Float64}, mech_dir::Vector{Float64})
  θplus = ComponentVector(p_net=θ.p_net .+ h .* p_dir, mech_raw=θ.mech_raw .+ h .* mech_dir)
  θminus = ComponentVector(p_net=θ.p_net .- h .* p_dir, mech_raw=θ.mech_raw .- h .* mech_dir)
  lplus = loss_fn(θplus)
  lminus = loss_fn(θminus)
  return lplus, lminus, (lplus - lminus) / (2h)
end
...
rng = StableRNG(42)
lines = String[]
push!(lines, "AFM03 Check-4: direct FD chain check for ks_raw / cs_raw / p_net -> train loss")
...
for rank in selected_ranks
  ...
  p_rand = randn(rng, length(p_net_vec))
  p_rand ./= norm(p_rand)
  ...
  for win in windows
    ...
    loss_fn = θ -> loss_single_ss_nomonitor(θ, ode_train, x2dot_train, contact_train, times_train,
      state12_scale_full, x2dot_scale_full, nn, st, known_pars, x3_t0_val,
      re_pnet, ks_bounds, cs_bounds; nn_gain=g_nn)

    loss0 = loss_fn(θ0)
    push!(lines, "  Window $(win.role) [$(win.start_idx), $(win.stop_idx)] len=$(win.len) base_loss=$(fmt_num(loss0))")

    for h in (1e-4, 1e-3)
      lpk, lmk, dks = fd_dir(loss_fn, θ0, h, zeros(length(p_net_vec)), [1.0, 0.0])
      lpc, lmc, dcs = fd_dir(loss_fn, θ0, h, zeros(length(p_net_vec)), [0.0, 1.0])
      lpp, lmp, dpn = fd_dir(loss_fn, θ0, h, p_rand, [0.0, 0.0])
      push!(lines, "    h=$(fmt_num(h, sigdigits=3)) | dL/dks_raw≈$(fmt_num(dks))  dL/dcs_raw≈$(fmt_num(dcs))  dL/dp_rand≈$(fmt_num(dpn))")
      push!(lines, "      ks_raw   : l+= $(fmt_num(lpk)) l-= $(fmt_num(lmk))")
      push!(lines, "      cs_raw   : l+= $(fmt_num(lpc)) l-= $(fmt_num(lmc))")
      push!(lines, "      p_rand   : l+= $(fmt_num(lpp)) l-= $(fmt_num(lmp))")
    end
  end
  push!(lines, "")
end
...
out_file = joinpath(out_dir, "afm03_check4_direct_fd_chain.txt")
