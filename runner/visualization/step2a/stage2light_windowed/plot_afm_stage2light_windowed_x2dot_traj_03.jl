...
const DEFAULT_OUT_FILE = "afm_param_stage2light_windowed_03_x2_x2dot_traj_grid.png"
...
function make_uode_func_oop(nn_weights, known_pars; nn_gain::Float64=1.0)
    ...
    function f(u, p, t)
        ...
        du3 = (-F_contact - ks_local * u[3]) / cs_local
        return [du1, du2, du3]
    end
    return f
end

function true_x2dot_from_state(u, t::Float64)
    F_contact = true_fcontact_from_state(u)
    return (Fd * cos(wd * t) - k * u[1] - c * u[2] + F_contact) / m
end

function predicted_x2dot_from_state(u, t::Float64, nn_weights, mech; nn_gain::Float64=1.0)
    s = dist + u[1] - u[3]
    w_pred = contact_weight(s, adhesion_transition)
...
function reconstruct_window_prediction(path::String, ode_post, traj_post)
    ...
    return (
        ...
        role = role,
        times_us = times_us,
        x2_true = vec(ode_win[2, :]),
        x2_pred = vec(uhat[2, :]),
        x2dot_true = Float64.([true_x2dot_from_state(view(ode_win, :, j), times[j]) for j in axes(ode_win, 2)]),
        x2dot_pred = Float64.([predicted_x2dot_from_state(view(uhat, :, j), times[j], nn_weights, mech; nn_gain=nn_gain) for j in axes(uhat, 2)]),
    )
end
...
function run_one(result_dir::String, out_dir::String)
    ...
    for rec in recs
        push!(panels, build_panel(rec.times_us, rec.x2_true, rec.x2_pred, rec.title, "x2"))
    end

    mkpath(out_dir)
    out_path = stage2light_checkpoint_plot_path(out_dir, DEFAULT_OUT_FILE, recs)
    fig = plot(panels...; layout = (2, 3), size = (1800, 980), margin = 8 * Plots.mm)
