...
const DEFAULT_OUT_FILE = "afm_param_stage2light_windowed_03_nnraw_wpred_grid.png"
...
function reconstruct_window_signals(path::String, ode_post, traj_post)
    ...
    nn_raw = Float64[]
    fcontact_true = Float64[]
    w_pred = Float64[]
    w_true = Float64[]
    for j in axes(uhat, 2)
        u = view(uhat, :, j)
        push!(nn_raw, nn_forward(nn_weights, nn_input_from_state(u))[1])
        push!(fcontact_true, true_fcontact_from_state(view(ode_win, :, j)))
        push!(w_pred, contact_weight(dist + u[1] - u[3], adhesion_transition))
        u_true = view(ode_win, :, j)
        push!(w_true, contact_weight(dist + u_true[1] - u_true[3], adhesion_transition))
    end
...
function nice_epoch_plot(times_us, y, title, ylabel; color=:black, label="")
    return plot(
        times_us,
        y;
        linewidth = 2,
        ...
        xlabel = "time (μs)",
        ylabel = ylabel,
        legend = label == "" ? false : :topright,
    )
end

function build_w_panel(times_us, w_pred, w_true, title)
    p = plot(
        times_us,
        w_true;
        linewidth = 2,
    ...
    plot!(p, times_us, w_pred; linewidth = 2, color = :darkgreen, linestyle = :dash, label = "w_pred")
    return p
end
...
function run_one(result_dir::String, out_dir::String)
    ...
    recs = [reconstruct_window_signals(path, post.ode, post.traj) for path in paths]

    panels = Plots.Plot[]
    for rec in recs
        push!(panels, nice_epoch_plot(rec.times_us, rec.nn_raw, rec.title, "NN_raw(t)"; color=:crimson, label="NN_raw"))
    end
    for rec in recs
        push!(panels, nice_epoch_plot(rec.times_us, rec.fcontact_true, rec.title, "F_contact true"; color=:black, label="F_contact"))
    end
    for rec in recs
        push!(panels, build_w_panel(rec.times_us, rec.w_pred, rec.w_true, rec.title))
    end
