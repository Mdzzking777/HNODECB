...
const DEFAULT_OUT_FILE = "afm_param_stage2light_windowed_03_fcontact_grid.png"
...
function predicted_fcontact_from_state(u, nn_weights; nn_gain::Float64=1.0)
    s = dist + u[1] - u[3]
    w_pred = contact_weight(s, adhesion_transition)
    nn_raw = nn_forward(nn_weights, nn_input_from_state(u))[1]
    return nn_gain * nn_raw * w_pred
end

function trapz_integral(t::AbstractVector{<:Real}, y::AbstractVector{<:Real})
    n = length(t)
    n == length(y) || error("Time and signal lengths do not match for trapezoidal integration.")
    n >= 2 || return 0.0
    acc = 0.0
...
function overlap_ratio(t::AbstractVector{<:Real}, y_true::AbstractVector{<:Real}, y_pred::AbstractVector{<:Real})
    length(t) == length(y_true) == length(y_pred) || error("Input lengths do not match for overlap computation.")
    y_min = min.(Float64.(y_true), Float64.(y_pred))
    y_max = max.(Float64.(y_true), Float64.(y_pred))
    denom = trapz_integral(t, y_max)
    denom == 0.0 && return NaN
    return trapz_integral(t, y_min) / denom
end
...
function reconstruct_window_fcontact(path::String, ode_post, traj_post)
    ...
    fcontact_true = [true_fcontact_from_state(view(ode_win, :, j)) for j in axes(ode_win, 2)]
    fcontact_pred = [predicted_fcontact_from_state(view(uhat, :, j), nn_weights; nn_gain=nn_gain) for j in axes(uhat, 2)]
    area_true = trapz_integral(times, fcontact_true)
    area_pred = trapz_integral(times, fcontact_pred)
    area_ratio = area_true == 0.0 ? NaN : area_pred / area_true
    overlap = overlap_ratio(times, fcontact_true, fcontact_pred)
    ...
    return (
        ...
        title = title,
        times_us = times_us,
        fcontact_true = Float64.(fcontact_true),
        fcontact_pred = Float64.(fcontact_pred),
        area_ratio = area_ratio,
        overlap = overlap,
    )
end

function build_panel(times_us, fcontact_true, fcontact_pred, title, area_ratio, overlap)
    p = plot(
        times_us,
        fcontact_true;
        linewidth = 2,
        color = :black,
        label = "F_contact true",
        title = title,
        xlabel = "time (μs)",
        ylabel = "force (N)",
        legend = :topright,
    )
    plot!(p, times_us, fcontact_pred; linewidth = 2, color = :crimson, linestyle = :dash, label = "NN raw * w_pred")
    x_annot = times_us[1] + 0.04 * (times_us[end] - times_us[1])
    y_min = min(minimum(fcontact_true), minimum(fcontact_pred))
    y_max = max(maximum(fcontact_true), maximum(fcontact_pred))
    y_span = max(y_max - y_min, eps(Float64))
    y_annot1 = y_max - 0.10 * y_span
    y_annot2 = y_max - 0.24 * y_span
    annotate!(p, x_annot, y_annot1, text(@sprintf("area ratio = %.3f", area_ratio), 10, :black, :left))
    annotate!(p, x_annot, y_annot2, text(@sprintf("overlap = %.3f", overlap), 10, :black, :left))
    return p
end
...
function run_one(result_dir::String, out_dir::String)
    ...
    recs = [reconstruct_window_fcontact(path, post.ode, post.traj) for path in paths]
    panels = [build_panel(rec.times_us, rec.fcontact_true, rec.fcontact_pred, rec.title, rec.area_ratio, rec.overlap) for rec in recs]
