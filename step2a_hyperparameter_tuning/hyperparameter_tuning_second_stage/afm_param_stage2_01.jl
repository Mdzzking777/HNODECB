...
using Serialization, DifferentialEquations, LinearAlgebra, DataFrames, Statistics, Printf
...
x3_range_eps = 1e-9
x3_range_weight = 1.0
# x3 initial condition at first-contact is assumed known in scenario 01
x3_init_known = ode_data[3, 1]
x3_init_weight = x3_contact_weight
...
best_ref = Ref((loss=Inf, ks=NaN, cs=NaN, Estar=NaN, state=Inf, x2dot=Inf, x3_init=Inf, x3_range=Inf,
...
for (seed_idx, seed) in enumerate(seed_candidates)
  ...
  for dks in grid_offsets, dcs in grid_offsets, dE in grid_offsets
    ...
    push!(stage2_results, (loss=loss, state=comps.state, x2dot=comps.x2dot, x3_init=comps.x3_init, x3_range=comps.x3_range,
    ...
    if loss < best_ref[].loss
      best_ref[] = (loss=loss, ks=ks, cs=cs, Estar=Estar,
        state=comps.state, x2dot=comps.x2dot, x3_init=comps.x3_init, x3_range=comps.x3_range,
...
# Oracle sanity check under the same loss definition (diagnostic only)
...
println("Stage2 oracle sanity (01) -- loss: ", @sprintf("%.4e", oracle_loss),
  " | state=", @sprintf("%.4e", oracle_comps.state),
  " x2dot=", @sprintf("%.4e", oracle_comps.x2dot),
  " x3_init=", @sprintf("%.4e", oracle_comps.x3_init),
